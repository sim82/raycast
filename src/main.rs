...
impl Mainloop {
    fn spawn(spawn: SpawnInfo) -> Mainloop {
        let mut map_dynamic;
        let level_id;
        let mut player;
        let mut things;
    ...
    fn run(&mut self, input_events: &InputState, buffer: &mut [u8]) {
        ...
        sprite::draw(sprite_screen_setup, &mut buffer[..], &zbuffer, &self.resources);
